...
5. Press **▶ Click each row & grab**. For each listing it clicks, then **waits until the detail panel actually shows THAT business** — it matches the panel's title against the row's name (via a MutationObserver, so it reacts the instant the panel populates). Only then does it read the fields. This confirms the *right* listing loaded, so it can't grab the previous one's phone/website, and it doesn't read a half-loaded panel. If the panel doesn't show the right business, it re-clicks up to 3×. Pauses by the **click delay** between listings (default 250 ms — raise it if Google rate-limits you). Progress bar + live "currently doing" line. **Stop** any time.
   - **Automatic retries:** after the first pass it **auto-re-runs the empties-&-duplicates retry up to 3 times** (stops early once there's nothing left), so you don't have to.
   - The **↻ Retry: N empty · M dup** button is still there for a manual pass (now showing empties and duplicates separately). Duplicate values are highlighted **light red** in the Preview.
