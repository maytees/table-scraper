...
5. Press **▶ Click each row & grab**. It clicks every listing, waits for the panel to switch, then reads each field **only once the field is a new element or its value changed** — so it never grabs the previous listing's phone/website before Google repaints. Pauses by the **click delay** (its own input — default 250 ms). Progress bar + live "currently doing" line. **Stop** any time. If some grabs came back empty **or duplicated** (a duplicate phone/website across distinct listings usually means a stale grab), a **↻ Retry empties & dups (N)** button appears and re-clicks just those rows. Duplicate values are highlighted **light red** in the Preview so you can spot them.
