...
5. Press **▶ Click each row & grab**. It clicks every listing, waits for the panel to actually switch (so it never grabs the previous listing's phone), pauses by the **click delay** (its own input — default 250 ms, separate from the scroll delay), then reads the value and fills the column. Progress bar + live counts in the status bar. **Stop** any time — re-running only processes rows still missing a value. If some grabs came back empty, a **↻ Retry empties (N)** button appears and re-clicks just those rows.
