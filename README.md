...
4. **Step 2 — teach the phone field.** Manually click any one listing so its detail panel opens. Press **＋ Pick detail field** — now hovering highlights elements on the page. Click the phone number. A name box appears (pre-filled, e.g. `phone`) — press **Save**. You can add more fields the same way (address, website, …) and remove them with ✕.
5. Press **▶ Click each row & grab**. It clicks every listing (random delay between clicks), waits for the detail panel to actually change before reading the value (so it never grabs the previous listing's phone), and fills the column. Progress bar + live counts in the status bar. **Stop** any time — re-running only processes rows still missing a value. If some grabs came back empty, a **↻ Retry empties (N)** button appears and re-clicks just those rows.
...
- **Listings without a phone number** wait up to ~5 s before being left blank — a pass with many phone-less listings is slower; that's normal.
